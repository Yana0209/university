...
function findAvailableClassrooms(timeSlot, dayOfWeek) {
    ...
    return classrooms
        ...
        .filter(function (number) { return !occupiedClassrooms.includes(number); });
}
...
function getClassroomUtilization(classroomNumber) {
    var totalLessonsInClassroom = schedule.filter(function (lesson) { return lesson.classroomNumber === classroomNumber; }).length;
...
function getScheduleDisplay() {
    return schedule.map(function (lesson) {
        var course = courses.find(function (c) { return c.id === lesson.courseId; });
        var professor = professors.find(function (p) { return p.id === lesson.professorId; });
        return "\n            Lesson Name: ".concat(lesson.name, ", \n            Professor: ").concat(professor ? professor.name : 'Unknown', ", \n            Classroom: ").concat(lesson.classroomNumber, ", \n            Day: ").concat(lesson.dayOfWeek, ", \n            Time: ").concat(lesson.timeSlot, "\n        ");
    }).join('<br>');
}
...
(_a = document.getElementById('addProfessorForm')) === null || _a === void 0 ? void 0 : _a.addEventListener('submit', function (event) {
...
(_b = document.getElementById('addLessonForm')) === null || _b === void 0 ? void 0 : _b.addEventListener('submit', function (event) {
    event.preventDefault();
